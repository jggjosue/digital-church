...
import { type PrayerDocument } from '@/lib/prayers';
...
const privacyIcons: { [key: string]: React.ReactNode } = {
  ...
  'Grupo Específico': <Users className="h-4 w-4 text-muted-foreground" />,
};
...
export default function PrayerRequestsPage() {
  ...
  const filteredData = React.useMemo(() => {
    ...
    if (selectedTab === 'my-requests') {
        data = data.filter((req: PrayerDocument) => req.submittedBy === userName);
    } else if (selectedTab === 'answered') {
        data = data.filter((req: PrayerDocument) => req.status === 'Respondido');
    }

    if (activeFilters.status.length > 0) {
        data = data.filter((req: PrayerDocument) => activeFilters.status.includes(req.status));
    }
    ...
    if (search.trim()) {
        ...
        data = data.filter((req: PrayerDocument) => 
    ...
    return data;
  }, [prayers, activeFilters, selectedTab, search, userName]);

  const renderTableBody = (data: PrayerDocument[]) => {
    if (isLoading) {
      return (
        <TableBody>
          <TableRow>
    ...
    return (
      <TableBody>
        {data.map((request) => {
          ...
          const dateStr = new Date(request.createdAt).toLocaleDateString('es-ES', { 
          ...
          return (
            <TableRow key={request.id}>
              ...
              <TableCell>
                ...
                <div className="text-sm text-muted-foreground max-w-sm truncate">{request.description}</div>
              </TableCell>
              <TableCell>
            ...
            <Tabs defaultValue="all-requests" onValueChange={setSelectedTab}>
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-4">
                     <div className="relative w-full max-w-sm">
                        ...
                        <Input 
                          placeholder="Buscar peticiones o nombres..." 
                          className="pl-9"
                          value={search}
